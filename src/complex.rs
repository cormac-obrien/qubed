...
pub trait NumAssign
    : Num + AddAssign<Self> + SubAssign<Self> + MulAssign<Self> + DivAssign<Self>
    {
}

impl NumAssign for i8 {}
impl NumAssign for i16 {}
impl NumAssign for i32 {}
impl NumAssign for i64 {}
impl NumAssign for isize {}
impl NumAssign for u8 {}
impl NumAssign for u16 {}
impl NumAssign for u32 {}
impl NumAssign for u64 {}
impl NumAssign for usize {}
impl NumAssign for f32 {}
impl NumAssign for f64 {}
...
impl<T: Clone + NumAssign> Complex<T> {
...
impl<T: Clone + NumAssign + Neg<Output = T>> Complex<T> {
...
impl<T: Clone + Float + NumAssign> Complex<T> {
    /// Calculate |self|
    #[inline]
    pub fn norm(&self) -> T {
    ...
    pub fn powc(&self, exp: Complex<T>) -> Complex<T> {
        ...
        let (r, theta) = self.to_polar();
        Complex::from_polar(&(r.powf(exp.re) * (-exp.im * theta).exp()),
    ...
    pub fn sin(&self) -> Complex<T> {
        ...
        Complex::new(self.re.sin() * self.im.cosh(),
    ...
    pub fn cos(&self) -> Complex<T> {
        ...
        Complex::new(self.re.cos() * self.im.cosh(),
    ...
    pub fn sinh(&self) -> Complex<T> {
        ...
        Complex::new(self.re.sinh() * self.im.cos(),
    ...
    pub fn cosh(&self) -> Complex<T> {
        ...
        Complex::new(self.re.cosh() * self.im.cos(),
...
impl<T: Clone + NumAssign> From<T> for Complex<T> {
    #[inline]
...
impl<'a, T: Clone + NumAssign> From<&'a T> for Complex<T> {
    #[inline]
    fn from(re: &T) -> Complex<T> {
        From::from(re.clone())
    }
...
macro_rules! forward_ref_ref_binop {
    (impl $imp:ident, $method:ident) => {
        impl<'a, 'b, T: Clone + NumAssign> $imp<&'b Complex<T>> for &'a Complex<T> {
...
macro_rules! forward_ref_val_binop {
    (impl $imp:ident, $method:ident) => {
        impl<'a, T: Clone + NumAssign> $imp<Complex<T>> for &'a Complex<T> {
            type Output = Complex<T>;
...
impl<T: Clone + NumAssign> Add<Complex<T>> for Complex<T> {
    type Output = Complex<T>;
...
// (a + i b) - (c + i d) == (a - c) + i (b - d)
impl<T: Clone + NumAssign> Sub<Complex<T>> for Complex<T> {
    type Output = Complex<T>;
...
impl<T: Clone + NumAssign> Mul<Complex<T>> for Complex<T> {
    type Output = Complex<T>;
...
impl<T: Clone + NumAssign> Div<Complex<T>> for Complex<T> {
    type Output = Complex<T>;
...
impl<T: Clone + NumAssign + Neg<Output = T>> Neg for Complex<T> {
    type Output = Complex<T>;
...
impl<'a, T: Clone + NumAssign + Neg<Output = T>> Neg for &'a Complex<T> {
    type Output = Complex<T>;
...
macro_rules! real_arithmetic {
    (@forward $imp:ident::$method:ident for $($real:ident),*) => (
        ...
        impl<'a, T: Clone + NumAssign> $imp<T> for &'a Complex<T> {
            type Output = Complex<T>;
        ...
        impl<'a, 'b, T: Clone + NumAssign> $imp<&'a T> for &'b Complex<T> {
            type Output = Complex<T>;
    ...
    (@implement $imp:ident::$method:ident for $($real:ident),*) => (
        impl<T: Clone + NumAssign> $imp<T> for Complex<T> {
            type Output = Complex<T>;
...
// constants
impl<T: Clone + NumAssign> Zero for Complex<T> {
    #[inline]
    fn zero() -> Complex<T> {
...
impl<T: Clone + NumAssign> One for Complex<T> {
    #[inline]
    fn one() -> Complex<T> {
...
impl<T> fmt::Display for Complex<T>
    where T: fmt::Display + NumAssign + PartialOrd + Clone
{
...
impl<T> fmt::LowerExp for Complex<T>
    where T: fmt::LowerExp + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<T> fmt::UpperExp for Complex<T>
    where T: fmt::UpperExp + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write_complex!(f, "E", "", self.re, self.im, T)
    }
...
impl<T> fmt::LowerHex for Complex<T>
    where T: fmt::LowerHex + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<T> fmt::UpperHex for Complex<T>
    where T: fmt::UpperHex + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write_complex!(f, "X", "0x", self.re, self.im, T)
    }
...
impl<T> fmt::Octal for Complex<T>
    where T: fmt::Octal + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<T> fmt::Binary for Complex<T>
    where T: fmt::Binary + Num + PartialOrd + Clone
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<T> serde::Deserialize for Complex<T>
    where T: serde::Deserialize + NumAssign + Clone
{
...
mod test {
    ...
    pub const _neg1_1i: Complex64 = Complex {
        re: -1.0,
        im: 1.0,
    };
    ...
    fn test_sqrt() {
        ...
        for &c in all_consts.iter() {
            ...
            assert!(-f64::consts::PI / 2.0 <= c.sqrt().arg() &&
    ...
    fn test_asin() {
        ...
        assert!(close(_1_0i.scale(-1.0).asin(),
    ...
    fn test_atan() {
        ...
        assert!(close(_1_0i.scale(-1.0).atan(),
    ...
    fn test_tanh() {
        ...
        assert!(close(_1_0i.tanh(),
                      _1_0i.scale((f64::consts::E.powi(2) - 1.0) /
                                  (f64::consts::E.powi(2) + 1.0))));
    ...
    fn test_asinh() {
        ...
        assert!(close(_0_1i.asinh().scale(-1.0),
        ...
        for &c in all_consts.iter() {
            ...
            assert!(-f64::consts::PI / 2.0 <= c.asinh().im &&
                    c.asinh().im <= f64::consts::PI / 2.0);
        }
    }
    ...
    fn test_acosh() {
        ...
        for &c in all_consts.iter() {
            ...
            assert!(-f64::consts::PI <= c.acosh().im && c.acosh().im <= f64::consts::PI &&
                    0.0 <= c.cosh().re);
        }
    }
    ...
    fn test_atanh() {
        ...
        for &c in all_consts.iter() {
            ...
            assert!(-f64::consts::PI / 2.0 <= c.atanh().im &&
                    c.atanh().im <= f64::consts::PI / 2.0);
        }
    }
    ...
    fn test_trig_identities() {
        for &c in all_consts.iter() {
            ...
            // sin(z) = (e^(iz) - e^(-iz))/(2i)
            assert!(close(((_0_1i * c).exp() - (_0_1i * c).exp().inv()) / _0_1i.scale(2.0),
                          c.sin()));
            // cos(z) = (e^(iz) + e^(-iz))/2
            assert!(close(((_0_1i * c).exp() + (_0_1i * c).exp().inv()).unscale(2.0),
                          c.cos()));
            // tan(z) = i (1 - e^(2iz))/(1 + e^(2iz))
            assert!(close(_0_1i * (_1_0i - (_0_1i * c).scale(2.0).exp()) /
    ...
    fn test_hyperbolic_identites() {
        for &c in all_consts.iter() {
            ...
            assert!(close((c.scale(2.0).exp() - _1_0i) / (c.scale(2.0).exp() + _1_0i),
                          c.tanh()));
        }
    }
    ...
    fn test_hash() {
        ...
        assert!(::complex::hash(&a) != ::complex::hash(&b));
        assert!(::complex::hash(&b) != ::complex::hash(&c));
        assert!(::complex::hash(&c) != ::complex::hash(&a));
    }
